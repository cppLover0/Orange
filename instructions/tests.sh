...
install() {
    ...
    x86_64-orange-mlibc-gcc "${tests_dir}"/timerfd_test.c -o "${dest_dir}"/usr/bin/timerfd_test
  ...
  -Wl,--build-id=sha1 -Wl,-z,noexecstack -Wl,-z,text -Wl,-z,relro -Wl,-z,now -Wl,-z,nocopyreloc
    ...
    x86_64-orange-mlibc-gcc "${tests_dir}"/locale_test.c -o "${dest_dir}/usr/bin/locale_test"
    clang --target=x86_64-orange-mlibc "${tests_dir}"/c_compiler_test.c -o "${dest_dir}"/usr/bin/clang_test 
    sh "${build_support}"/run-wrapper "${dest_dir}"/usr/bin/locale_test

...
configure() {
    CFLAGS="-D__NR_fchmodat2=452" CXXFLAGS="$CFLAGS" meson --libdir=/usr/lib --prefix=/usr "${source_dir}" -Dlinux_kernel_headers="${dest_dir}/usr/include" -Dposix_option=enabled -Dlinux_option=enabled -Dglibc_option=enabled -Dbsd_option=enabled
}

...
long long sys_socket(int family, int type, int protocol) {
    ...
    type = type & 0xF;
    if(type != SOCK_STREAM) {
        log("sockets", "unimplemented socket type %d for family %d", type, family);
        return -EINVAL;
    }

    if(family != PF_UNIX) {
        //log("sockets", "unimplemented socket family %d for type %d", family, type);
    } 
...
long long sys_connect(int fd, const struct sockaddr *addr_ptr, std::uint32_t addr_length) {
    ...
    if(file->socket.socket_type == PF_UNIX) {
        ...
        long long ret = unix_sockets::connect(file, (sockaddr_un*)addr_ptr);
...
long long sys_msg_send(int fd, struct msghdr* hdr, int flags) {
    ...
    if(fd_s->socket.socket_type == PF_UNIX) {
        ...
        for (cmsg = CMSG_FIRSTHDR(msg);
            ...
            if (cmsg->cmsg_level == SOL_SOCKET && cmsg->cmsg_type == SCM_RIGHTS) {
                ...
                *tmp_ucred = *tm;
                ...
                unix_socket_node* node = (unix_socket_node*)fd_s->socket.socket_pointer;

                if(fd_s->socket.socket_side == 1)
                    fd_s->socket.un.r_ucred->push((uint64_t)node);
                else 
                    fd_s->socket.un.w_ucred->push((uint64_t)node);
            }
        }
        ...
        klibc::debug_printf("msg_send fd %d total_written %lli flags %d from proc %d\n",fd,total_written,flags,proc->id);

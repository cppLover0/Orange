...
uint64_t* __paging_next_level(std::uint64_t* table,std::uint16_t idx,std::uint64_t flags) {
    if(!(table[idx] & PTE_PRESENT))
        table[idx] = memory::pmm::_physical::alloc(4096) | flags;
...
void memory::paging::map(std::uint64_t cr3,std::uint64_t phys,std::uint64_t virt,std::uint64_t flags) {
    ...
    if(PTE_INDEX(align_virt,39) < 256)
        new_flags |= PTE_USER;
    uint64_t* pml3 = __paging_next_level(cr30,PTE_INDEX(align_virt,39),new_flags);
    uint64_t* pml2 = __paging_next_level(pml3,PTE_INDEX(align_virt,30),new_flags);
    uint64_t* pml = __paging_next_level(pml2,PTE_INDEX(align_virt,21),new_flags);
...
void __paging_map_kernel(std::uint64_t cr3) {
    ...
    for(std::uint64_t i = ALIGNDOWN((std::uint64_t)&kernel_start,4096);i < ALIGNUP((std::uint64_t)&kernel_end,4096);i += 4096) {
        memory::paging::map(cr3,i - ker->virtual_base + ker->physical_base,i,PTE_PRESENT | PTE_RW);
    }

}

void memory::paging::mapkernel(std::uint64_t cr3) {
    __paging_map_kernel(cr3);
}
...
void memory::paging::alwaysmappedmap(std::uint64_t cr3) {
    alwaysmapped_t* current = alwmap;
    while(current) {
        maprange(cr3,current->phys,(std::uint64_t)Other::toVirt(current->phys),current->len,PTE_RW | PTE_PRESENT);  

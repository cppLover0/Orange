...
char __hid_to_tty_kbd[] = {
    ...
    [0x29] = '\0',[0x2A] = 127,[0x2B] = '\t',[0x2C] = ' ',[0x2D] = '-',[0x2E] = '=',
...
void __usbkeyboard_send_tty(uint8_t add,uint8_t key) {
    if(key > 3 && key < 0x48) {
        char tty_key = __hid_to_tty_kbd[key];
        if(add == 2 || add == 32) {
...
void __usbkeyboard_handler(xhci_usb_device_t* usbdev,xhci_done_trb_t* trb) {
    ...
    for(int i = 2;i < 8;i++) {
        ...
        if(success || data[i] == 0x2A)
            __usbkeyboard_send_tty(add,data[i]);
        success = 1;
    }

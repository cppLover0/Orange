...
extern "C" void CPanic(const char* msg,int_frame_t* frame) {
    ...
    NLog("\nReason: %s\nCPU: %d\nVector: %d/0x%p (error code: %d/0x%p)\n",msg,Lapic::ID(),frame->vec,frame->vec,frame->err_code,frame->err_code);
    NLog("Registers dump\n");
    ...
    frame->rip,frame->ss,frame->cs,frame->rflags,frame->rsp,frame->rax,frame->rbx,frame->rcx,frame->rdx,frame->rsi,frame->rdi,frame->rbp,frame->r8,frame->r9,frame->r10,cr2,frame->r11,frame->r12,frame->r13,frame->r14,frame->r15);
    __hlt();
}

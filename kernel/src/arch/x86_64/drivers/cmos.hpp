...
namespace x86_64 {
    namespace cmos {
        static inline std::uint8_t cmos_read(std::uint8_t reg) {
            x86_64::io::outb(CMOS_ADDRESS, reg);
            return x86_64::io::inb(CMOS_DATA);
        }
        ...
        static inline int daysInMonth(int month, int year) {
        ...
        static inline static std::uint64_t get_unix_time_cmos() {
            int year = cmos::year();
